...
const PART_URLS = Array.from(
  { length: 8 },
  (_, index) => `./assets/showa/master/part-${String(index).padStart(2, '0')}.txt?v=20260819-1925`
);
...
async function loadSceneDataUrl() {
  setStatus('正在載入完整昭和場景… 0/8');

  const parts = [];
  for (let index = 0; index < PART_URLS.length; index += 1) {
    const response = await fetch(PART_URLS[index], { cache: 'no-store' });
    if (!response.ok) {
      throw new Error(`背景分段 ${index + 1}/8 載入失敗（HTTP ${response.status}）`);
    }

    const part = (await response.text()).replace(/\s+/g, '');
    if (!part) throw new Error(`背景分段 ${index + 1}/8 是空白資料`);
    parts.push(part);
    setStatus(`正在載入完整昭和場景… ${index + 1}/8`);
  }
