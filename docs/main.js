...
const WORLD_H = TILE_H;
const MIN_ZOOM = 0.35;
const MAX_ZOOM = 3.0;

const TILE_URLS = [
  './assets/showa/scene-left.jpg',
  './assets/showa/scene-right.jpg'
];
...
class ShowaLittleWorld extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.image(0, 0, 'showa-0').setOrigin(0, 0).setDepth(0);
    this.add.image(TILE_W, 0, 'showa-1').setOrigin(0, 0).setDepth(0);

    this.hotspots = [
      { name: '喫茶店吧檯', x: 455, y: 405 },
      { name: '戶外桌位', x: 560, y: 590 },
      { name: '公園長椅', x: 1220, y: 675 },
      { name: '大樹下', x: 970, y: 430 }
    ];

    this.hotspots.forEach(h => {
      const zone = this.add.zone(h.x, h.y, 150, 110).setInteractive().setDepth(20);
      zone.on('pointerdown', () => {
        const el = document.getElementById('status');
        if (el) el.textContent = `${h.name} · future Agent social hotspot`;
      });
    });
