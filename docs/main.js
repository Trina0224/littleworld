...
const MIN_ZOOM = 0.45;
const MAX_ZOOM = 3.0;
const ASSET_BASE = 'https://raw.githubusercontent.com/Trina0224/littleworld/main/assets/showa/';
const TILE_URLS = ['tile-00.svg','tile-01.svg','tile-02.svg','tile-10.svg','tile-11.svg','tile-12.svg'].map(f => ASSET_BASE + f);

class ShowaLittleWorld extends Phaser.Scene {
  constructor() { super('ShowaLittleWorld'); this.dragging=false; this.lastPointer=null; this.lastPinchDistance=0; }
  preload() { TILE_URLS.forEach((url,i)=>this.load.image(`showa-${i}`,url)); }
  create() {
    ...
    for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++) {
      const i=r*COLS+c;
      this.add.image(c*TILE_W,r*TILE_H,`showa-${i}`).setOrigin(0,0).setDepth(0);
    }
    this.hotspots=[
    ...
    this.hotspots.forEach(h=>{
      ...
      zone.on('pointerdown',()=>{ const el=document.getElementById('status'); if(el) el.textContent=`${h.name} · 未來 Agent social hotspot`; });
    });
    this.setupCameraControls(); this.setupZoomButtons(); this.fitScene();
  }
  fitScene() {
    const cam=this.cameras.main;
  ...
  setupCameraControls() {
    const cam=this.cameras.main; this.input.addPointer(2);
    this.input.on('pointerdown',pointer=>{
      const active=this.input.manager.pointers.filter(p=>p.isDown);
      if(active.length>=2){ this.dragging=false; this.lastPinchDistance=Phaser.Math.Distance.Between(active[0].x,active[0].y,active[1].x,active[1].y); return; }
      this.dragging=true; this.lastPointer={x:pointer.x,y:pointer.y};
    });
    ...
    this.input.on('pointermove',pointer=>{
      const active=this.input.manager.pointers.filter(p=>p.isDown);
      if(active.length>=2){
        const [a,b]=active; const d=Phaser.Math.Distance.Between(a.x,a.y,b.x,b.y);
        if(this.lastPinchDistance>0) this.zoomAt((a.x+b.x)/2,(a.y+b.y)/2,cam.zoom*d/this.lastPinchDistance);
        this.lastPinchDistance=d; this.dragging=false; return;
      }
      if(!this.dragging||!this.lastPointer) return;
      const dx=pointer.x-this.lastPointer.x, dy=pointer.y-this.lastPointer.y;
      cam.scrollX-=dx/cam.zoom; cam.scrollY-=dy/cam.zoom; this.lastPointer={x:pointer.x,y:pointer.y};
    });
    this.input.on('wheel',(pointer,objs,dx,dy)=>this.zoomAt(pointer.x,pointer.y,cam.zoom-dy*0.0008));
  }
  setupZoomButtons() {
    const cam=this.cameras.main, cx=()=>this.scale.width/2, cy=()=>this.scale.height/2;
...
const game=new Phaser.Game({
  ...
  backgroundColor:'#1d261c',scene:ShowaLittleWorld,input:{activePointers:3},
